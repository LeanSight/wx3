...
```
normalize -> enhance -> transcribe -> srt -> compress
```

`compress_step` opera sobre `ctx.src` (el video original) pero utiliza el "mejor audio logrado" (enhanced > normalized > src) para el muxing final.
El step detecta cual usar via `ctx.media_type`.

**Reutilizacion de componentes (estilo declarativo)**:

```python
_NORMALIZE = NamedStep(
...
- `wx41/steps/compress.py`: usa `ctx.video_out` when `ctx.media_type == "audio"`
