...
### D9: Typer no esta en requirements.txt

`requirements.txt` no incluye `typer`. El CLI propuesto en la seccion 6 lo
requiere como nueva dependencia. Typer tiene dos variantes de instalacion:

- `typer` (sin extras): instala solo el core, sin autocompletado de shell
- `typer[all]`: incluye `rich` para help coloreado y `shellingham` para autocompletado

`rich` ya esta en `requirements.txt` sin version pinneada.

**Duda**: Agregar `typer[all]` (aprovecha el rich ya instalado) o solo `typer`?
Y con que version constraint (el proyecto no usa pins estrictos para rich/transformers)?

### D10: Version del SDK de AssemblyAI y disponibilidad de Transcriber.submit()

`assemblyai` no aparece en `requirements.txt`. El SDK se instala por separado
(probablemente como dependencia de `transcribe_aai.py` actual). El metodo
`Transcriber.submit()` (submit no-bloqueante) fue introducido en el SDK de
Python de AssemblyAI a partir de la version 0.28.0.

Si la version instalada es anterior a 0.28.0, `submit()` no existe y el
progreso de transcripcion no es posible sin reescribir `transcribe_aai.py`
para usar la API REST directamente.

**Duda**: Que version del SDK de AssemblyAI esta instalada? Si es < 0.28.0,
la seccion 3.9 y el D7 deben revisarse: la alternativa es mantener
`transcribe_assemblyai` sincrono (sin progress_callback funcional) y mostrar
solo un spinner indeterminado en la UI.
