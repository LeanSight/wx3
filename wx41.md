...
## 7. Decisiones de diseno resueltas

### D1: Mecanismo de skip y resume

**Resolucion**: `PipelineState` en `step_common.py` (ver seccion 3.2).
- Elimina `cache_check_step`, `cache_save_step`, `ctx.cache_hit`, `ctx.cache`
- Estado persistido en `{stem}.wx41.json` por archivo fuente
- Recuerda que steps el usuario configuro para saltar (user_skipped_steps)
- `--force` ignora el estado y re-corre todo desde cero

### D2: CLI framework

**Resolucion**: Typer. Reduce boilerplate de definicion de CLI a la mitad.
Usa type hints, compatible con el estilo Python 2026 del proyecto.

### D3: Ubicacion de helpers compartidos de steps

**Resolucion**: `wx41/step_common.py`. Contiene `@timer`, `atomic_output`,
`temp_files`, `run_compression`, `PipelineState`. Se agregan modulos adicionales
si son necesarios para mantener codigo modular y sencillo.

### D4: PipelineObserver: Protocol vs event hooks

**Resolucion**: Protocol. Genera codigo mas sencillo: quien implementa declara
todos los metodos en un lugar, la verificacion de tipo estatica detecta
implementaciones incompletas, `isinstance` funciona en runtime. Los event hooks
individuales requieren N registros y no hay garantia de completitud.

### D5: Signal handling

**Resolucion**: `signal.signal()` en `main()` seteando un `threading.Event`.
El loop de archivos chequea el evento antes de cada archivo. Los `try/finally`
en steps garantizan limpieza de temporales en cualquier modo de interrupcion.
No se necesita signal handling en `Pipeline` ni en steps individuales.

### D6: dry_run con --force

**Resolucion**: dry_run ignora `--force` siempre. Muestra el estado natural
del proceso (que ya esta hecho vs que falta), que es la pregunta util al
querer entender que haria wx41 al reprocesar. `--force` solo aplica a `run()`.

### D7: Progreso de transcripcion AssemblyAI

**Resolucion**: Usar `Transcriber.submit()` + polling manual cada 3 segundos
(ver seccion 3.9). Los 3 estados reportados son: queued (0/3), processing (1/3),
completed (3/3). `transcribe_aai.py` acepta `progress_callback` opcional para
no romper el contrato con wx4.

### D8: Multi-archivo y observers

**Resolucion**: Stateless. Una instancia de `Pipeline` por archivo (construida
por `MediaOrchestrator._build_pipeline()`). `RichPipelineObserver` tiene un
metodo `reset()` que limpia `_task_ids` entre archivos. Una sola instancia del
observer se reutiliza en el loop multi-archivo.
