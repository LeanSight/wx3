...
def black_video_step(ctx: PipelineContext, config: BlackVideoConfig) -> PipelineContext:
    ...
    out_path = predict_output_path(audio, "black_video", config.output_keys[0])
...
def black_video_output_fn(ctx: PipelineContext, config: BlackVideoConfig) -> Dict[str, Path]:
    if not config.enabled:
        return {}
    audio = ctx.outputs.get("enhanced") or ctx.outputs.get("normalized") or ctx.src
    out_path = predict_output_path(audio, "black_video", config.output_keys[0])
