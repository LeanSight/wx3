...
def _instantiate_step(step_name: str, config: PipelineConfig, all_steps: Dict[str, Any]) -> Optional[NamedStep]:
...
def build_video_pipeline(config: PipelineConfig, observers: List[PipelineObserver]) -> Pipeline:
    all_steps = get_all_steps()
...
class MediaOrchestrator:
    ...
    def run(self, src: Path, dry_run: bool = False) -> PipelineContext:
        ...
        ctx = PipelineContext(src=src, media_type=media_type, force=self._config.force, dry_run=dry_run)
