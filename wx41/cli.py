...
def create_cli():
    ...
    @click.option(
        "--dry-run", is_flag=True, help="Simulate execution without running steps"
    )
    @click.option(
        "--force", is_flag=True, help="Force re-execution even if outputs exist"
    )
    @click.pass_context
    def main(ctx, src: Path, dry_run: bool, force: bool, **kwargs):
        ...
        if info.config_class:
            for field in dataclasses.fields(info.config_class):
                ...
                main = click.option(
                    ...
                    and field.type in (str, int, float, bool, Path)
                    else None,
                    help=help_text,
                )(main)
